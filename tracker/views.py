...
def get_weighted_random_movie(unseen_movies):
    """
    Selects a movie tier based on weights, then returns a random
    movie from that tier. Returns None if the tier is empty.
    """
    tiers = {
...
def next_movie_view(request):
    ...
    if request.method == 'POST':
        ...
        # Preserve BOTH filters after a swipe
    ...
    # --- GET LOGIC: Handles both filters ---
...
def profile_view(request):
    ...
    total_seen_movies = UserMovieView.objects.filter(
        user=user,
        has_seen=True
    ).count()
